...
{   
    ...
    while ((position = strstr(str, Old_Word)) != NULL)              // `strstr` Returns pointer to first occurrence of str2 in str1
    {
        // Copying the Current Line
        strcpy(temp, str);                                          // 'strcpy' Copies str2 into str1

       // Getting the Index of first occurrence of `oldWord` in `str` 
        index = position - str;
        ...
        strcat(str, New_Word);                                      // `strcat` Concatenates str2 at the end of str1
...
{   
    ...
    // Read each line from Main file and write to destination file after replacing the given word.
