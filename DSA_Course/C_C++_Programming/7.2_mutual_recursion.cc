...
// Informing the Complier that a function named 'xmove' will be defined Later. 
...
int omove(char board[SIZE][SIZE]){
    ...
    for(i=0; i<SIZE;i++){
        for(j=0 ; j<SIZE; j++ ){
            ...
            if (isWin(board,'o')==1){board[i][j] = BLANK; return(i*3+j+1); }  // (i*3+j+1) is a trick for converting index notation to 1-9
            //BASE CASE
