...
Input
First line contains a single integer N, the length of A
Second line contains N space separated strings A[i]

Output
Print the longest common prefix found in all strings. If there is no common prefix print -1.
*/
...
string common_prefix_of_two_strings(string str1, string str2)
{
	...
	for (int i = 0, j = 0; i <= len1 - 1 && j <= len2 - 1; i++, j++)
	{
		if (str1[i] != str2[j])
			break;
		common_prefix.push_back(str1[i]);
	}
...
string get_longest_common_prefix(string arr[], int low, int high)
{
	...
	if (high > low)
	{
		// Avoids overflow for large low and high values, but it is equal to (low + high)/2.
