...
namespace number_theory{
    ...
    //int ncrModPFermat(int n,int r, int p)    { if (n < r ){ return 0;} if (r == 0){ return 1; } int fac[n + 1]; fac[0] = 1; fr(i,1, n) { fac[i] = (fac[i - 1] * i) % p; } return (fac[n] * modInverse(fac[r], p) % p * modInverse(fac[n - r], p) % p) % p;}
}
